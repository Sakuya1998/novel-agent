...
async def test_writing_loop_routing(orch):
    ...
    assert await orch.decide_next({**base, "current_phase": "writing"}) == "scene_writer"
...
def test_route_from_orchestrator():
    ...
    assert route_from_orchestrator({}) == "scene_writer"  # 缺省安全回退
